...
CEngineState *ToState(EngineHandle h) { return static_cast<CEngineState *>(h); }
...
void engine_mass_spawn(EngineHandle engine, const char *scene_name, int count,
  ...
  for (int i = 0; i < count; ++i) {
    std::string name =
        std::string(prefix ? prefix : "obj") + "_" + std::to_string(i);
    ...
    state.facade.GetEventBus().Subscribe<std::string>(
        [&state, id](const std::string &data) {
          ...
          if (it != state.callbacks.end()) {
            ...
            void *ud = (ud_it != state.callback_user_data.end()) ? ud_it->second
                                                                 : nullptr;
