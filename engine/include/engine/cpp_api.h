...
template <typename T> T *GetComponent(GameObject &go) {
...
inline std::vector<std::shared_ptr<GameObject>>
FindObjectsByName(const Scene &scene, std::string_view name) {
...
template <typename EventT> void Publish(EventBus &bus, const EventT &event) {
  bus.Publish(event);
}
...
CreateSceneWithDefaultObject(EngineFacade &engine,
...
inline std::vector<std::shared_ptr<GameObject>>
BatchCreateObjects(std::shared_ptr<Scene> scene, int count,
                   const std::string &prefix) {
  std::vector<std::shared_ptr<GameObject>> result;
  result.reserve(count);
  for (int i = 0; i < count; ++i) {
    auto obj = scene->CreateObject(prefix + "_" + std::to_string(i));
    obj->AddComponent<TransformComponent>();
    result.push_back(obj);
  }
  return result;
...
inline std::optional<std::shared_ptr<Scene>>
FindScene(EngineFacade &engine, const std::string &name) {
