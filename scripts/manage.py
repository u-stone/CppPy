...
def _get_includes_for_tidy():
    """Return include flags for clang-tidy when compile_commands.json is absent.

    Detects Python, pybind11, nanobind paths from the venv and 3rdparty.
    """
    includes = [
        os.path.join(PROJECT_ROOT, "engine", "include"),
    ]
    python = _get_venv_python() if os.path.exists(_get_venv_python()) else sys.executable
    try:
        ...
        py_include = result.stdout.strip()
        if py_include:
            includes.append(py_include)
    except Exception:
        pass
    # 3rdparty includes
    for name in ["pybind11", "nanobind"]:
        d = os.path.join(PROJECT_ROOT, "3rdparty", name, "include")
        if os.path.isdir(d):
            includes.append(d)
    return includes
...
def cmd_tidy(args):
    ...
    Prefers compile_commands.json (generated by Ninja).  Falls back to
    manual include paths for Visual Studio generators.
    """
    ...
    if os.path.exists(compile_db):
        print("[tidy] Using compile_commands.json")
        base_cmd = ["run-clang-tidy", "-p", BUILD_DIR]
    else:
        print("[tidy] No compile_commands.json (VS generator) — using manual includes")
        extra_args = ["-std=c++17"]
